...
//		if(CCSE.config.OtherMods.Horticookie){
			...
			Horticookie.config.juicyHelper = config.juicyHelper;
...
//		}
	...
	Horticookie.getMenuString = function(){
		...
		str += '<div class="listing">' + WriteButton('juicyHelper', 'juicyHelperButton', 'Juicy Helper ON', 'Juicy Helper OFF', '') + '<label>Automatically harvests plants that can sprout in a JQB Slot (Duketater + Shriekbulb).</label></div>';
			...
			if(M.isTileUnlocked(x, y)){
				...
				if(tile[0] != 0){
					...
					if(tile[1] >= plant.mature){
						...
						if(Horticookie.config.juicyHelper && (plant.unlocked &&(plant.key == 'duketater' || plant.key == 'shriekbulb'))){
							M.clickTile(x, y);
							Horticookie.recalcTileStatus();
						}
						else{
